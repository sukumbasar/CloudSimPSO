...
public class Scheduler_PSO {
    ...
    public static List<Cloudlet> applyPSO(List<Cloudlet> cloudletList, List<Vm> vmList) {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            age[i] = 0;
            if (pBestFitness[i] < gBestFitness) { gBestFitness = pBestFitness[i]; gBest = particles[i].clone(); }
        }
        ...
        for (int iter = 0; iter < MAX_ITERATIONS; iter++) {
            ...
            for (int i = 0; i < PARTICLE_COUNT; i++) {
                ...
                if (fitness < pBestFitness[i]) { pBest[i] = particles[i].clone(); pBestFitness[i] = fitness; age[i] = 0; }
                else age[i]++;
                if (fitness < gBestFitness) { gBestFitness = fitness; gBest = particles[i].clone(); }
            }

            if (gBestFitness < previousBest) { previousBest = gBestFitness; noImprovementCount = 0; }
            else noImprovementCount++;
            if (noImprovementCount >= 20) { System.out.printf("  Early stopping: %d%n", iter); break; }
        }
    ...
    private static int[] generateOpposite(int[] position, int vmCount) {
        ...
        for (int j = 0; j < position.length; j++) opposite[j] = (vmCount - 1) - position[j];
        return opposite;
    }

    private static int[] generateMaxMinVariation(List<Cloudlet> cloudletList, List<Vm> vmList, int variationId) {
        int taskCount = cloudletList.size(); int vmCount = vmList.size();
        int[] solution = new int[taskCount]; double[] vmLoad = new double[vmCount];
